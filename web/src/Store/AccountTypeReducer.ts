...
export function setAccountTypesFetched(
  accountTypes: AccountTypeDto[],
...
export function setAccountTypesAdded(
  accountTypes: AccountTypeDto[],
): AccountTypeActionTypes {
  return {
...
export function setAccountTypeFetchError(
  error: string,
): AccountTypeActionTypes {
  return {
  ...
  switch (action.type) {
    ...
    case SET_ACCOUNT_TYPES_REMOVED:
      return {
        ...state,
        accountTypes: [
          ...state.accountTypes.filter((o) => !action.payload.includes(o.id)),
        ],
      };
