...
const _Navbar: React.FC<NavbarProps> = (
    ...
    const { user } = zcc.useAuth();
    const [navbarOpen, setNavbarOpen] = useState<boolean>(false);

    const closeNavbar = (): void => setNavbarOpen(false);

    return (
        <div data-testid="Navbar" className={`${styles.navbar} ${navbarOpen && styles.responsive}`}>
            {
                props.items.map(_ =>
                    <NavLink key={_.name} onClick={closeNavbar} to={_.route}>{_.name}</NavLink>
                )
            }
